...
**It is now federated multi-tenant**: one Worker + one D1 serve many people, each running an isolated System behind **their own BotFather bot** (bring-your-own token, multiplexed by webhook path). The original owner is tenant #1 and works unchanged. Every per-owner table carries a `user_id`; a scoped-`env` seam (`worker/src/tenant.js`) plus a fail-loud DB guard enforce isolation. **Read `docs/09-multi-tenant.md` before touching queries, routing, or `scheduled()`** — new D1 queries on per-owner tables MUST scope by `user_id` and bind `uid(env)`, or the guard (`env.TENANT_GUARD=warn|strict`) flags them.
...
- `tenant.js` — **the multi-tenant seam**. `scopeEnv(env, tenantRow)` wraps `env` once per entry point (tenant-guarded `DB`, `env._tenant`); accessors `uid`/`tz`/`ownerChat`/`botToken`/`webhookSecret`; `resolveTenant(env, by)` (by webhook_id / dashboard_token / notify_secret / id, owner via env secrets); `encryptSecret`/`decryptSecret` (AES-GCM, `env.MASTER_KEY`); `hexToken`; `syntheticOwner` (tenant #1 from env). See `docs/09-multi-tenant.md`.
...
- `system.js` — **The System**: `goals`/`quests`/`milestones`, XP/level/streak (in `state`), `clockContext` (temporal facts), `planGoal`/`adaptPlan` (roadmap planner — both reason over `goalContext`: owner profile + memories embedding-recalled against the goal + latest `health`/`metrics` numbers with 30-day trends + keyword-matched finished research reports + one live web search via `search.js` (Serper → CSE → DDG → Wikipedia, all fail-soft), and must produce measurable milestones each carrying 3-6 concrete day-sized `steps` (JSON column, migration 006) that quest generation issues from; both may emit up to 3 `plan_questions` back to the owner — answers arrive via chat tool `answer_plan_question`, the Plans tab, or `POST /api/plan-question`, become memories, and re-plan the goal immediately), `maybeAdaptOnDone` (re-evaluate a plan when a quest is cleared, 4h per-goal cooldown shared with the nightly pass via `adapt_last_<id>`), milestone-aware quest generation (sees the last 30 quests *with status* — a ✗ failed step gets re-issued, not treated as covered — plus the shared `measuredNumbers` latest-value+30d-trend block), `computeProgress`/`paceOf`, `issueDaily`, `debrief` (auto-fail, streak, re-tunes every drifted/failed/milestone-hit goal ≤3/night), `checkAwards`/`grantAward`/`rankTitle`/`awardCatalog` (one-time awards — streaks, quest totals, E→S hunter-rank promotions, per-goal 30-day transformation; idempotent via UNIQUE key, runs nightly + after every cleared quest; `awardCatalog` merges earned + still-locked thresholds with progress for the dashboard's always-visible Awards ladder), `autonomyTick` (the bounded daily ponder — mode/budget/quiet-hours guarded), `runSystem` (cron entry), and `SYSTEM_TOOLS`. The strict-mentor motive lives here.
...
- `life.js` — money: bank-notification → `transactions`, accounts/holdings/health/people tools, plus calorie tracking (`log_meal`/`nutrition` → `meals` table, charted on the dashboard's Life tab).
