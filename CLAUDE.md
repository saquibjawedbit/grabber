...
- `system.js` — **The System**: `goals`/`quests`/`milestones`, XP/level/streak (in `state`), `clockContext` (temporal facts), `planGoal` (roadmap planner), milestone-aware quest generation, `computeProgress`/`paceOf`, `issueDaily`, `debrief`, `autonomyTick` (the bounded daily ponder — mode/budget/quiet-hours guarded), `runSystem` (cron entry), and `SYSTEM_TOOLS`. The strict-mentor motive lives here.
