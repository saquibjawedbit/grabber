...
| `plan_questions` | The planner's questions back to the owner (facts it needs to plan better) | `recordPlanQuestions()` on plan/adapt; answered via chat tool, Plans tab, or `POST /api/plan-question` | `status` `open\|answered\|dismissed`; ≤3 open per goal, deduped; an answer also becomes a memory and triggers an immediate `adaptPlan`; migration 008. |
| `awards` | One-time recognitions — streaks, quest totals, rank promotions, 30-day transformations | `grantAward()`/`checkAwards()` in `system.js` (nightly debrief + after every ✅) | `key` UNIQUE + INSERT OR IGNORE → idempotent grants; carries bonus `xp`, optional `goal_id`; each also carries a tangible real-world `reward` (memory-generated, tier-scaled) + `reward_claimed_at` (migration 010); migration 007. |
| `milestones` | The persistent **roadmap** per goal | `planGoal()` in `system.js` | ordered `seq`; `done_when`, `steps` (JSON array of concrete day-sized actions — the quest source material, migration 006), `target_date`, `status` `pending\|active\|done\|skipped`. Daily quests are issued from the `active` milestone's steps. |
