...
| `/api/award-claim` | POST | Mark an award's tangible reward redeemed: `{id}` claims (stamps `reward_claimed_at`), `{id,unclaim:true}` reverses a mis-tap → `claimReward`. Powers the Awards card's **Claim your reward ✓** button. |
| `/api/system` | GET | The dashboard **System tab** payload: `rank` (level/XP/streak + `title` hunter rank), `goals` (each with `progress`, `pace`, `projected`, its `milestones` roadmap, and `plan_reasoning`/`plan_at`), `quests_today`, `quest_history` (per-IST-day done/failed counts, last 14 days — the Battle record chart), `activity` (the work log, newest 60, with `actor`/`reasoning`), `awards` (earned only) plus `award_catalog` (the full ladder — earned + still-locked with `current`/`target`/`pct` progress, so the Awards panel shows what's next instead of hiding until the first threshold crosses; earned entries also carry the tangible `reward`, its `id`, and `reward_claimed_at`), `plan_questions` (open), and `settings` (autonomy mode + budget). |
